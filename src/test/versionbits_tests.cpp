...
BOOST_AUTO_TEST_CASE(versionbits_computeblockversion)
{
    ...
    const auto chainParams = CreateChainParams(CBaseChainParams::MAIN);
    const Consensus::Params &mainnetParams = chainParams->GetConsensus();
    const int nWindow = mainnetParams.nMinerConfirmationWindow;
    ...
    lastBlock = firstChain.Mine(nWindow, nTime, VERSIONBITS_LAST_OLD_BLOCK_VERSION).Tip();
    BOOST_CHECK_EQUAL(ComputeBlockVersion(lastBlock, mainnetParams) & (1<<bit), 0);

    // Mine more blocks at the old time, and check that CBV isn't setting the bit yet.
    for (int i=1; i<(nWindow-4); i++) {
        lastBlock = firstChain.Mine(nWindow+i, nTime, VERSIONBITS_LAST_OLD_BLOCK_VERSION).Tip();
    ...
    nTime = nStartTime;
    for (int i=(nWindow-4); i<=nWindow; i++) {
        lastBlock = firstChain.Mine(nWindow+i, nTime, VERSIONBITS_LAST_OLD_BLOCK_VERSION).Tip();
        BOOST_CHECK_EQUAL(ComputeBlockVersion(lastBlock, mainnetParams) & (1<<bit), 0);
    }

    // Advance to the next period and transition to STARTED,
    lastBlock = firstChain.Mine(nWindow*3, nTime, VERSIONBITS_LAST_OLD_BLOCK_VERSION).Tip();
    ...
    nTime += 600;
    int blocksToMine = nWindow * 2; // test blocks for up to 2 time periods
    int nHeight = nWindow * 3;
    ...
    // next period.
    lastBlock = secondChain.Mine(nWindow, nStartTime, VERSIONBITS_LAST_OLD_BLOCK_VERSION).Tip();
    BOOST_CHECK((ComputeBlockVersion(lastBlock, mainnetParams) & (1<<bit)) != 0);

    // Mine another period worth of blocks, signaling the new bit.
    lastBlock = secondChain.Mine(nWindow*2, nStartTime, VERSIONBITS_TOP_BITS | (1<<bit)).Tip();
    ...
    lastBlock = secondChain.Mine((nWindow*3)-1, nStartTime, VERSIONBITS_LAST_OLD_BLOCK_VERSION).Tip();
    BOOST_CHECK((ComputeBlockVersion(lastBlock, mainnetParams) & (1<<bit)) != 0);
    lastBlock = secondChain.Mine(nWindow*3, nStartTime, VERSIONBITS_LAST_OLD_BLOCK_VERSION).Tip();
    BOOST_CHECK_EQUAL(ComputeBlockVersion(lastBlock, mainnetParams) & (1<<bit), 0);
